// Cascade — feature / override test (kitchen sink). Not shown in the viewer; this
// is the exhaustive exercise of per-component overrides, per-call args, the auto
// filter, justify, and the rhythm helpers. The clean shared specimen is sample.typ.
//   typst compile --root . cascade-typst/feature-test.typ

#import "lib.typ": layout, rhythm

// Options come from `--input` (the viewer's Compile button passes the selected
// scale / body / heading / theme). Defaults mirror the CSS sample, so `just pdf`
// with no inputs produces the viewer's baseline.
#let _in = sys.inputs
