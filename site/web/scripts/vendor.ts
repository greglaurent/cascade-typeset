// Downloads the pinned datastar bundle into src/vendor/ so Vite can hash and bundle it like any
// other asset. datastar's npm package is frozen at a beta; the v1 line ships only as a prebuilt
// bundle, so this one file is fetched here. Bump the version and re-run (the build does this
// automatically via the `prebuild` step).
