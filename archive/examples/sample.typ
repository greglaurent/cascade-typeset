// Cascade — shared specimen. Mirrors examples/sample.html word-for-word so the CSS and
// Typst renderers show the same document. The text is a short, self-referential essay
// on setting type that exercises every component.
//
// Options come from `--input`; defaults mirror the CSS sample, so `just pdf` with no
// inputs is the baseline. For a kitchen-sink test of per-call overrides and rhythm
// helpers, see feature-test.typ.
